...
COMPANY_KEYWORD = 'new products'
# COMPANY_KEYWORD = 'new products' # 21 rows, 18 got company names, one got government name as company name, two could not determine company
...
class RSSer():
    ...
    def search_keyword(self, keyword, date_start=None, date_end=None):
        ...
        if date_start is None:
            query = self.base_url.format('"' + keyword.strip() + '" "' + COMPANY_KEYWORD + '"')
            # query = self.base_url.format('"' + keyword.strip() + '"')
        else:
            if self.time_url is not None:
                query = self.time_url.format('"' + keyword.strip() + '" "' + COMPANY_KEYWORD + '"', date_start, date_end)
                # query = self.time_url.format('"' + keyword.strip() + '"', date_start, date_end)
            else:
        ...
        for keyword in tqdm(args.keywords):
            ...
            if r is not None:
                ...
                if args.outdir is not None:
                    ...
                    filename = '%s_%s_%s_%s_%s.csv' % (datetime.now().strftime(
                        '%Y-%m-%d'), source, keyword.replace(' ','-'), start_date_str, end_date_str)
...
if __name__ == '__main__':
    ...
    if args.input_file:
        args.keywords = ''.join(
            open(args.input_file, 'r').readlines()).strip().split('\n')
    main(args)
