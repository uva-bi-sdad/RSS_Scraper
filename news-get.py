...
class NewsGetter:
    ...
    def text_from_url(self, url, sleep=(0, 0.1)):
        ...
        if len(output_string) < 400:
            ...
            if self.severity < 2:
                self.connection_error.append('Cannot scrape text from ' + url)
        ...
        return re.sub("\s+", " ", output_string)

    def sum_from_url(self, url, sleep=(0, 0.1), **kwargs):
        print("Trying sum from url")
        time.sleep(random.uniform(sleep[0], sleep[1]))
        try:
            sumy_parser = HtmlParser.from_url(url, self.tokenizer)
        except:
    ...
    def texts_from_csv(self, csv_dir, summarize=False, thread_count=16, json_lines=False, filter_string="", exclude_filter=""):
                ...
                while True:
                    ...
                    if summarize:
                        ...
                        try:
                            ...
                            print("news_getter.text_from_url bombed out!")
                            text = null
        ...
        for file in csv_files:
            ...
            if df.shape[0] == 0:
                print(f"RSS {file} has no rows, skipping")
                continue
            if json_lines:
                outdir = self.outdir + "/" + file.split("/")[-1].replace("csv", "json")
            else:    
            ...
            for worker in workers:
                texts = pd.concat([texts, worker.result])
            df['text'] = texts

            # Remove lines where text is null
            df_cleansed = df.loc[df["text"].str.len() > 0]

            # Apply the article filter, if applicable
            if filter_string != "":
                df_cleansed["filter_flag"] = numpy.where(df_cleansed.text.str.contains(filter_string, case=False),1,0)
                df_cleansed = df_cleansed.loc[df_cleansed["filter_flag"] == 1]

            # Apply the exclude filter, if applicable
            if exclude_filter != "":
                print(exclude_filter)
                df_cleansed["exclude_flag"] = numpy.where(df_cleansed.text.str.contains(exclude_filter, case=False),1,0)
                df_cleansed = df_cleansed.loc[df_cleansed["exclude_flag"] != 1]

            if df_cleansed.shape[0] > 0:
                if json_lines:
                    df_cleansed.to_json(outdir, orient='records', lines=True)
                else:    
                    df_cleansed.to_csv(outdir, mode='w+', index=False)
            print('file saved to ' + outdir)

def make_filter(filter_file):
    if filter_file == "":
        # no filter file defined
        return ""

    # open the filter file and read it into a list
    ...
    # convert filter_list to a string
    filter_string = '|'.join(filter_list)
    return filter_string
            
def main(args):
    getter = NewsGetter(severity=args.severity, output_dir=args.output_dir, language='english')
    filter_string = make_filter(args.article_filter)
    exclude_filter = make_filter(args.exclude_filter)

    if args.dir_to_csv is not None:
        getter.texts_from_csv(args.dir_to_csv, args.summarize, args.threads_count, args.json_lines, filter_string, exclude_filter)
    if args.urls is not None:
        ...
        for url in args.urls:
            if args.summarize:
                ...
                urls_output = pd.concat([urls_output, pd.DataFrame([retrieved], columns=urls_output.columns)])
        if args.json_lines:
            urls_output.to_json(args.output_dir + "/" + 'url_output.csv', orient='records', lines=True) 
        else:
            urls_output.to_csv(args.output_dir + "/" + 'url_output.csv', index=False)
...
def check_args(args):
    ...
    if args.json_lines is None:
        args.json_lines = False

    if args.article_filter is None:
        args.article_filter = ""
    elif not os.path.isfile(args.article_filter):
        raise ValueError('Article directory (%s) is not a valid directory' % (
            os.path.abspath(args.article_filter)))

    if args.exclude_filter is None:
        args.exclude_filter = ""
    elif not os.path.isfile(args.exclude_filter):
        raise ValueError('Article directory (%s) is not a valid directory' % (
            os.path.abspath(args.exclude_filter)))
...
if __name__ == '__main__':
    ...
    parser.add_argument('-f', '--article_filter', help='optional file with list of keywords for filtering article text', required=False)
    parser.add_argument('-x', '--exclude_filter', help='optional file with list of keywords for excluding results', required=False)
